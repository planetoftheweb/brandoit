...
*   **⬅️➡️ Carousel viewer:** Step the main viewer through your recent-generations history one tile at a time with prev/next arrow buttons or ←/→ keyboard shortcuts. Arrows stay clear of the comparison slider when it's open.
*   **🗂️ Customizable gallery:** Recent Generations supports a thumbnail size dropdown (X-Small / Small / Medium / Large) and an optional details panel under each tile (prompt + tag chips). Both preferences persist to localStorage. Per-tile action buttons stay out of the way until you hover.
*   **📁 Folders:** Group generation tiles into named folders with an always-present `Inbox`, a sticky pin that controls which folder new tiles land in, a tab strip to filter the gallery, bulk "Move to folder" from selection mode, and rename / delete actions (deleting a folder sweeps its tiles into Inbox). Folders persist on the user document for signed-in users and in `localStorage` for guests, so the gallery looks the same before and after sign-in.
